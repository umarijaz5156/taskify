...
class MeetingsController extends Controller
{
    ...
    public function list()
    {
        ...
        $meetings = $this->user->hasRole('admin') ? $this->workspace->meetings() : $this->user->meetings();
        if ($search) {
        ...
        $meetings = $meetings->orderBy($sort, $order)
            ...
            ->through(
                fn ($meeting) => [
                    'id' => $meeting->id,
                    'title' => $meeting->title,
                    'start_date_time' => format_date($meeting->start_date_time, 'H:i:s'),
                    'end_date_time' => format_date($meeting->end_date_time, 'H:i:s'),
                    'users' => $meeting->users,
    ...
    public function destroy_multiple(Request $request)
    {
        ...
        foreach ($ids as $id) {
            DeletionService::delete(Meeting::class, $id, 'Task');
        }

        return response()->json(['error' => false, 'message' => 'Meetings(s) deleted successfully.']);
    }
    ...
    public function join($id)
    {
        ...
        if ($currentDateTime < $meeting->start_date_time) {
            ...
            if ($meeting->users->contains($this->user->id) || $this->user->hasRole('admin')) {
    ...
    public function duplicate($id)
    {
        ...
        if (!$duplicateMeeting) {
            if (request()->has('reload') && request()->input('reload') === 'true') {
                Session::flash('error', 'Meeting duplication failed.');
            }
        ...
        return response()->json(['error' => false, 'message' => 'Meeting duplicated successfully.']);
    }
}
