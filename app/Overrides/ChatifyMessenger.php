...
class ChatifyMessenger
{
    ...
    public function fetchMessagesQuery($user_id)
    {
        ...
        return Message::where($where)
            ->orWhere('from_id', $user_id)->where('to_id', Auth::user()->id)->where('workspace_id', session()->get('workspace_id'));
    }
    ...
    {
        $where = ['from_id' => $user_id, 'seen' => 0, 'to_id' => Auth::user()->id, 'workspace_id' => session()->get('workspace_id')];
        Message::Where($where)
            ->update(['seen' => 1]);
        return 1;
    }
    ...
    {
        return Message::where('seen', 0)->where('from_id', '!=', getAuthenticatedUser()->id)->where('to_id', getAuthenticatedUser()->id)->where('workspace_id', session()->get('workspace_id'))->count();
    }
    ...
    {
        $where = ['favorite_id' => $user_id, 'user_id' => Auth::user()->id, 'workspace_id' => session()->get('workspace_id')];
    ...
    public function makeInFavorite($user_id, $action)
    {
        if ($action > 0) {
            ...
            $star->workspace_id = session()->get('workspace_id');
