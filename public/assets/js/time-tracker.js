        ...
        $.ajax({
            ...
            success: function (result) {
                if (result["error"] == false) {
                    recorded_id = result["recorded_id"];
...
function pauseTimer() {
    if (is_timer_stopped == false && $("#second").val() > "00") {
        ...
        toastr.warning('Please make sure timer has started.');
    }
}
...
function stopTimer() {
    if (is_timer_stopped == false && $("#second").val() > "00") {
        $('#stopTimerModal').modal('show'); // show the confirmation modal
...
function timeSheetActionsFormatter(value, row, index) {
    return [
        '<button title=' + label_delete + ' type="button" class="btn delete" data-id=' + row.id + ' data-type="time-tracker">' +
