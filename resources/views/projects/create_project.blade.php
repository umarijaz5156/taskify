                    ...
                    @endif
                            <div class="row">
                                <div class="mb-3 col-md-6">
                                    <label class="form-label" for="status_id"><?= get_label('status', 'Status') ?> <span
                                            class="asterisk">*</span></label>
                                    <div class="input-group">
                                        <select class="form-select" id="status_id" name="status_id">
                                            @foreach ($statuses as $status)
                                                <option value="{{ $status->id }}" class="badge bg-label-{{ $status->color }}"
                                                    {{ old('status_id') == $status->id ? 'selected' : '' }}>{{ $status->title }}
                                                    ({{ $status->color }})</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="mt-2">
                                        ...
                                        <a href="/status/manage" target="_blank"><button type="button"
                                                class="btn btn-sm btn-primary" data-bs-toggle="tooltip" data-bs-placement="right"
                                                data-bs-original-title="<?= get_label('manage_status', 'Manage status') ?>"><i
                            ...
                            <div class="row">
                                <div class="mb-3">
                                   ...
                                   <div class="input-group">
    <select id="" class="form-control js-example-basic-multiple" name="client_id"
        data-placeholder="<?= get_label('type_to_search', 'Type to search') ?>">
        @foreach ($clients as $client)
            <?php 
                $selected = ($client->id == getAuthenticatedUser()->id && $auth_user->hasRole('client')) ? 'selected' : '';
                $oldValue = collect(old('client_id'))->first(); // Get the first value if it's an array
            ?>
            <option value="{{ $client->id }}"
                {{ $oldValue == $client->id ? 'selected' : '' }} {{ $selected }}>{{ $client->first_name }} {{ $client->last_name }}</option>
        @endforeach
    </select>
</div>
                            ...
                            <div class="row">
                                <div class="mb-3 col-md-12">
                                    ...
                                    <div class="input-group">
                                        <select id="" class="form-control js-example-basic-multiple" name="tag_ids[]"
                                            multiple="multiple"
                                            data-placeholder="<?= get_label('type_to_search', 'Type to search') ?>">
                                            @foreach ($tags as $tag)
                                                <option value="{{ $tag->id }}"
                                    ...
                                    <div class="mt-2">
                                        <a href="javascript:void(0);" data-bs-toggle="modal"
                                            data-bs-target="#create_tag_modal"><button type="button"
                                                class="btn btn-sm btn-primary" data-bs-toggle="tooltip" data-bs-placement="right"
                                                data-bs-original-title=" <?= get_label('create_tag', 'Create tag') ?>"><i
                                                    class="bx bx-plus"></i></button></a>
                                        <a href="/tags/manage"><button type="button" class="btn btn-sm btn-primary"
            ...
            <div class="row">
                ...
                <div class="mb-3 col-md-12">
                    ...
                    <textarea class="form-control" id="description" name="description" rows="5"
                        placeholder="Website URL: www.example.com, Login URL:http://www.example.com/login:, Username: admin. Pass: password123">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-danger text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
